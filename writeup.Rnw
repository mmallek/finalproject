...
\subsection{Focus of Analysis} We are most interested in the baby's weight at birth, which we will examine in both its continuous and factored format. We chose not to construct a specific hypothesis based on this data to allow for freedom and flexibility within the individual projects. We assumed that all variables collected were potentially important or significant as predictors, so they were all included at the initial phase of each individual analysis. The initial, full model (shown for both the continuous and categorical version of the variable-of-interest) is shown here:
...
Our individual analyses explore the relationships between variables and seek to define the relative importance of some variables, and eventually a best model. The first analysis presented here involved writing a function to rank and identify the best model for the observed data using AIC, BIC, and $R^2$. A second exploration of variable selection and model diagnostics complemented the first project and added an investigation into leverage using Cook's distance. The third project focused on the potential utility of classification trees to help understand relative variable importance and provide a more intuitive, visual model of the data. Finally, the last project sought to develop methods for addressing missing values in the dataset and examine how the presence of missing values may affect results.
...
\begin{tabular}{lllll}
\hline
Continuous Variable & Two-level Factor & Equation with factor labels & Observations & Missing Values\\
\hline
...
All individual analyses of the dataset found that weeks was the strongest predictor of weight for the babies. The multitude of techniques for model-building corroborate this conclusion, and it makes intuitive sense that more developed babies in the womb for more time would have a healthier weight than babies born prematurely. In terms of leverage, the dataset did not show any significant observations that would influence the model significantly. 

One of the main challenges from this dataset is that we do not have data synced to a gestational age other than at term. The values for \verb|visits|, \verb|weeks|, \verb|weight|, and \verb|gained| are all collected at birth. While it makes sense to include all variables in any exploratory analysis of relationships among these variables, the value of any of these as prediction terms is problematic because they are collected at the end of the pregancy. Future studies should include data over time to enhance predictive power of any models developed.

Regardless of the individual approach chosen, an appreciation of the complex decisions, and the potentially subtle or enormous ramifications of those decisions, was developed by each member of the team. This dataset was difficult to analyze in a way that generated interesting and meaningful conclusions, which was an unexpected outcome. However, this provided an additional lesson in the importance of careful study design, as the potential for any dataset to answer the questions asked by the researcher is clearly highly constrained by the methodology employed in data collection.
