...
#fancyRpartPlot(pbt3, main="Pruned Classification Tree for Low Birth Weight", uniform=FALSE)
#fancyRpartPlot(pbt3)
#prp(bt3, type=1, extra=101, uniform=FALSE)

#bt4 = rpart(form1, data=births, method="class")
#fancyRpartPlot(bt4)
#cparam = bt4$cptable[which.min(bt4$cptable[,"xerror"]),"CP"]
#pbt4 = prune(bt4, cp=cparam)
#fancyRpartPlot(pbt4)
@

%<<echo=FALSE, fig.height=4, fig.width=4, message=FALSE, warning=FALSE, fig.cap='Gini Coefficient-Based Tree Output %and Diagram for Pruned Classification Tree'>>=
%fancyRpartPlot(bt3)
%@
