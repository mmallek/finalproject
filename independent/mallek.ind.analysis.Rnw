...
<<prunedtree1, fig.height=4, fig.width=4, echo=FALSE, fig.cap="Classification tree using the full model to grow the tree. 10-fold cross-validation was used to prune the tree, and the final result is pictured here.">>=
form1 = lowbirthweight ~ fage + mage + mature + weeks + visits + marital + gained + gender + habit + whitemom
bt1 = tree(form1, data = births)
bt1.cv = cv.tree(bt1)
bt1.p = prune.tree(bt1, best=3)
...
<<echo=FALSE, fig.height=5, fig.width=6, message=FALSE, warning=FALSE, fig.cap='Gini Coefficient-Based Tree Output and Diagram for Pruned Classification Tree'>>=
fancyRpartPlot(bt3)
@
