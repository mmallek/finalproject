...
lm(weight ~ mage + fage + weeks + visits + gained + habit + whitemom + marital + gender, data=births)
@
...
\medskip
1. Creates a matrix where each $row_i$ corresponds to one possible combination of predictor variables \\
...
2. Iterate through the rows of the matrix and fit a linear model, with $weight$ as the response variable \\
<<echo=FALSE>>=
...
3. Find the models with smallest AIC and BIC values, and largest adjusted R squared value\\

% display orders here
% display one out[[]] just to show it
